                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr class="{{ !$doc->equipment ? 'table-danger' : '' }}">
                                ...
                                <td>
                                    {{ $doc->equipment ? $doc->equipment->product_name : '' }}
...
let selectedSubMenu = '';
let selectedTool = '';
let selectedToolData = null; // Untuk menyimpan data tool yang dipilih
...
function renderToolsList(tools) {
    ...
    // Dalam function yang load tools (misal: loadToolsBySubMenu)
    tools.forEach(tool => {
        const html = `
                <label class="tool-radio">
                    <input type="radio" name="tools" value="${tool.id}" onchange="handleToolSelect('${tool.id}', '${tool.name}', '${tool.id_column}')">
                    <div class="tool-info">
...
function handleToolSelect(toolId, toolName, toolIdColumn, subMenu) {
    selectedSubMenu = $('#subMenuSelect').val();
    selectedTool = toolId;
    document.querySelectorAll('.tool-radio').forEach(el => el.classList.remove('selected'));
    event.target.closest('.tool-radio').classList.add('selected');
    updateNextButton();
    loadDocumentTable(toolId, selectedSubMenu);
...
function updateNextButton() {
    if (selectedSubMenu && selectedTool) {
...
function goToStep2() {
    if (!selectedSubMenu || !selectedTool) {
        Swal.fire({
            icon: 'warning',
            title: 'Incomplete',
            text: 'Please select both sub-menu and tools',
    ...
    const menu = subMenus.find(m => m.id === selectedSubMenu);
    const tool = toolsData[selectedSubMenu].find(t => t.id == selectedTool);
    selectedToolData = tool; // Simpan data tool yang dipilih
    //debugging
    // console.log('Selected Tool Data:', selectedToolData);
    updateDocumentNumber(); // Update document number berdasarkan tool yang dipilih


    // Populate Step 2 with selected data
    $('#selectedToolsId').text(tool.id_column);
    $('#selectedToolsName').text(tool.name);
    $('#selectedCategory').text(menu.category);
    $('#selectedNoDoc').text(menu.no_doc);
    $('#toolsIdInput').val(tool.id);
    ...
    $('#modalSubtitle').text('Upload document details and file');
}
...
function updateDocumentNumber() {
    ...
    // Jika ada tool yang dipilih, tambahkan building, dept, dan serial number
    if (selectedToolData && selectedToolData.building && selectedToolData.department) {
        const building = selectedToolData.building.substring(0, 3).toUpperCase();
        const dept = selectedToolData.department.substring(0, 3).toUpperCase();
